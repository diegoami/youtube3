...
- **paths to normally ignore:** `samples/test/*.json` — captured API
  ...
  it). `liked*.json`, `unliked-*.json`, `reliked-*.json` — the owner's
  exported likes and the undo logs of rating runs: personal data, ignored by
  git; read one only to check an export's shape or count, and never paste

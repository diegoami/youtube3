...
| F-3 | "the liked" — "The convenience operations is what I missed" | in review | Proposal #20. Export the `LL` playlist to JSON, incrementally; bulk unlike through `videos.rate(rating="none")` by channel, date or id, with a dry run. Depends on F-2. |
